...
async def test_get_docs(*args) -> None:
...
async def test_serialize(raw, output) -> None:
    async with create_mongo_source() as source:
...
async def test_get_client_when_pass_conflicting_values(host_value, ssl_value) -> None:
