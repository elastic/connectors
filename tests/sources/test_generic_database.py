...
from connectors.sources.database_utils import CursorSync as BaseCursorSync
...
class CursorSync(BaseCursorSync):
    ...
    def __init__(self, query_object: MSSQLQueries, *args, **kwargs) -> None:
        """Setup dummy cursor"""
...
def test_configured_tables(tables: Union[str, List[str]], expected_tables: List[Union[str, Any]]) -> None:
...
def test_is_wildcard(tables: Union[List[str], str]) -> None:
...
def test_map_column_names(schema: Optional[str], tables: Optional[List[str]], prefix: str) -> None:
...
async def get_cursor(query_object: MSSQLQueries, query: None) -> CursorSync:
