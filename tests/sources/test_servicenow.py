...
def setup_servicenow(source):
    ...
    source.configuration.set_value(name="username", value="admin")
    source.configuration.set_value(name="password", value="changeme")
...
async def test_validate_config_missing_fields_then_raise(field):
    async with create_source(ServiceNowDataSource) as source:
        source.configuration.set_value(name=field, value="")
