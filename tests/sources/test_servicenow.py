...
class MockResponse:
    ...
    def __init__(self, res: bytes, headers: Dict[str, Union[str, int]]) -> None:
        """Setup a response"""
        self._res = res
...
@pytest.mark.asyncio
async def test_validate_configuration_with_invalid_service_then_raise() -> Iterator[None]:
    async with create_service_now_source() as source:
...
@pytest.mark.asyncio
async def test_ping_for_unsuccessful_connection_then_raise() -> Iterator[None]:
    async with create_service_now_source() as source:
        with mock.patch.object(
            ServiceNowClient,
            "get_table_length",
...
@pytest.mark.asyncio
async def test_get_data() -> Iterator[None]:
    async with create_service_now_source() as source:
...
async def test_get_data_with_retry() -> Iterator[Optional[Task]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
...
@pytest.mark.asyncio
async def test_get_table_length() -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(
                res=SAMPLE_RESPONSE,
...
@pytest.mark.asyncio
@patch("connectors.utils.time_to_sleep_between_retries", Mock(return_value=0))
async def test_get_table_length_with_retry() -> Iterator[Optional[Task]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            side_effect=ServerDisconnectedError
        )
...
@pytest.mark.asyncio
@patch("connectors.utils.time_to_sleep_between_retries", Mock(return_value=0))
async def test_get_data_with_empty_response() -> Iterator[Optional[Task]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(
                res=b"",
...
@pytest.mark.asyncio
@patch("connectors.utils.time_to_sleep_between_retries", Mock(return_value=0))
async def test_get_data_with_text_response() -> Iterator[Optional[Task]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(
                res=b"Text",
...
@pytest.mark.asyncio
async def test_filter_services_with_exception() -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client.services = ["label_1", "label_3"]
...
@pytest.mark.asyncio
async def test_filter_services_when_sysparm_fields_missing() -> Iterator[None]:
    async with create_service_now_source() as source:
...
@pytest.mark.asyncio
async def test_filter_services_when_sysparm_fields_missing_for_unrelated_table() -> Iterator[None]:
    async with create_service_now_source() as source:
...
@pytest.mark.asyncio
async def test_get_docs_with_skipping_table_data() -> Iterator[Union[Future, Awaitable, None]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(
                res=SAMPLE_RESPONSE,
...
async def test_get_docs_with_skipping_attachment_data(dls_enabled: bool, expected_response: Dict[str, Union[List[str], str]]) -> Iterator[Union[Future, Awaitable, None]]:
    async with create_service_now_source() as source:
...
@pytest.mark.asyncio
async def test_get_docs_with_configured_services() -> Iterator[Union[Future, Awaitable, None]]:
    async with create_service_now_source() as source:
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_with_doit() -> Iterator[Optional[Future]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_with_extraction_service() -> Iterator[Optional[Future]]:
    with (
        patch(
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_with_upper_extension() -> Iterator[Optional[Future]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(res=b"Attachment Content", headers={})
        )
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_without_doit() -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(res=b"Attachment Content", headers={})
        )
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_with_exception() -> Iterator[Optional[Future]]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            side_effect=Exception("Something went wrong")
        )
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_with_unsupported_extension_then_skip() -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(res=b"Attachment Content", headers={})
        )
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_without_extension_then_skip() -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(res=b"Attachment Content", headers={})
        )
...
@pytest.mark.asyncio
async def test_fetch_attachment_content_with_unsupported_file_size_then_skip() -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(res=b"Attachment Content", headers={})
        )
...
@pytest.mark.asyncio
async def test_advanced_rules_validation(advanced_rules: Union[List[Dict[str, str]], List[Dict[str, Union[List[str], str]]], List[Union[Dict[str, Union[List[str], str]], Dict[str, str]]]], expected_validation_result: SyncRuleValidationResult) -> Iterator[None]:
    async with create_service_now_source() as source:
        source.servicenow_client.get_table_length = mock.AsyncMock(return_value=2)
...
@pytest.mark.asyncio
async def test_get_docs_with_advanced_rules(filtering: connectors.protocol.connectors.Filter) -> Iterator[Union[Future, Awaitable, None]]:
    async with create_service_now_source() as source:
        source.servicenow_client.services = ["custom"]
        source.servicenow_client._api_call = mock.AsyncMock(
            return_value=MockResponse(
...
@pytest.mark.asyncio
async def test_get_docs_with_advanced_rules_pagination(filtering: connectors.protocol.connectors.Filter) -> Iterator[Union[Future, Awaitable, None]]:
...
@pytest.mark.asyncio
async def test_fetch_access_control_for_public() -> Iterator[None]:
    async with create_service_now_source() as source:
        with mock.patch.object(
            ServiceNowDataSource,
            "_table_data_generator",
