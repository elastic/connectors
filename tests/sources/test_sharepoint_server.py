...
class MockResponse:
    ...
    def __init__(self, json, status) -> None:
        self._json = json
        self.status = status
...
@pytest.mark.asyncio
async def test_get_content_with_content_extraction() -> None:
    response_content = "This is a dummy sharepoint body response"
    with (
        patch(
...
@pytest.mark.asyncio
async def test_api_call_negative(patch_default_wait_multiplier) -> None:
...
@pytest.mark.asyncio
async def test_get_site_pages_content() -> None:
    EXPECTED_ATTACHMENT = {
        "id": 1,
        "server_relative_url": "/url",
        "_timestamp": "2022-06-20T10:37:44Z",
