...
class RedisClientMock:
    async def execute_command(self, SELECT: str="JSON.GET", key: Union[str, int]="json_key") -> str:
        return json.dumps({"1": "1", "2": "2"})

    async def zrange(self, key: str, start: int, skip: int, withscores: bool=True) -> Set[int]:
        return {1, 2, 3}

    async def smembers(self, key: str) -> Set[int]:
        return {1, 2, 3}

    async def get(self, key: str) -> str:
        return "this is value"

    async def hgetall(self, key: str) -> str:
        return "hash"

    async def xread(self, key: Dict[str, str]) -> str:
        return "stream"

    async def lrange(self, key: str, start: int, skip: int) -> List[int]:
        return [1, 2, 3]

    async def config_get(self, databases: str) -> Dict[str, str]:
        return {"databases": "1"}
    ...
    async def type(self, key: str) -> str:  # NOQA
        return "string"
...
async def test_get_key_value(key: str, key_type: str, expected_response: Union[List[int], str, Set[int], Dict[str, str]]) -> None:
    async with create_redis_source() as source:
        source.client.database = ["*"]
        source.client._client = RedisClientMock()
...
@pytest.mark.asyncio
@freeze_time("2023-01-24T04:07:19+00:00")
async def test_get_docs_with_sync_rules(filtering: connectors.protocol.connectors.Filter) -> None:
    async with create_redis_source() as source:
        source.client.database = ["*"]
...
@pytest.mark.asyncio
async def test_advanced_rules_validation(advanced_rules: Union[List[Dict[str, int]], Dict[str, Union[str, int]], List[Dict[str, Union[str, int]]], Dict[str, int], Dict[str, List[str]]], expected_validation_result: SyncRuleValidationResult) -> None:
    async with create_redis_source() as source:
        source.client._client = RedisClientMock()
