...
class TestMicrosoftSecurityToken:
    ...
    async def test_get_raises_correct_exception_when_any_other_status(self):
        ...
        token = TestMicrosoftSecurityToken.StubMicrosoftSecurityTokenWrongConfig(500, message)
...
class TestGraphAPIToken:
    ...
    async def token(self):
        return GraphAPIToken(aiohttp.ClientSession(), None, None, None, None)
    ...
    async def test_fetch_token(self, token, mock_responses):
        ...
        mock_responses.post(re.compile(".*"), payload={"access_token": bearer, "expires_in": str(expires_in)})
...
class TestSharepointRestAPIToken:
    @pytest_asyncio.fixture
    async def token(self):
        return SharepointRestAPIToken(aiohttp.ClientSession(), None, None, None, None)
...
class TestMicrosoftAPISession:
    ...
    async def microsoft_api_session(self):
        return MicrosoftAPISession(aiohttp.ClientSession(), TestMicrosoftAPISession.StubAPIToken(), self.scroll_field)
...
class TestSharepointOnlineClient:
    ...
    async def client(self):
        client = SharepointOnlineClient(self.tenant_id, self.tenant_name, self.client_id, self.client_secret)
        yield client
        await client.close()
    ...
    async def test_site_lists(self, client, patch_scroll):
        ...
        returned_items = await self._execute_scrolling_method(partial(client.site_lists, site_id), patch_scroll, actual_items)
    ...
    async def test_site_list_items(self, client, patch_scroll):
        ...
        returned_items = await self._execute_scrolling_method(partial(client.site_list_items, site_id, list_id), patch_scroll, actual_items)
    ...
    async def test_site_list_item_attachments(self, client, patch_fetch):
        ...
        returned_items = []
        async for attachment in client.site_list_item_attachments(site_web_url, list_title, list_item_id):
    ...
    async def test_site_pages(self, client, patch_scroll):
        ...
        returned_items = await self._execute_scrolling_method(partial(client.site_pages, page_url_path), patch_scroll, actual_items)
    ...
    @pytest.mark.asyncio
    async def test_get_tenant_details(self, client, patch_fetch):
        http_call_result = { "hello": "world" }

        patch_fetch.return_value = http_call_result

        actual_result = await client.get_tenant_details()
