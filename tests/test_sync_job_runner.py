...
def mock_sync_job(job_type, index_name) -> Mock:
    sync_job = Mock()
    sync_job.id = "1"
...
def create_runner_yielding_docs(docs=None) -> SyncJobRunner:
    if docs is None:
        docs = []
...
async def test_source_not_changed(
    job_type: JobType, sync_cursor_to_claim, sync_cursor_to_update
) -> None:
...
@pytest.mark.asyncio
async def test_invalid_filtering(job_type: JobType, sync_orchestrator_mock) -> None:
    ingestion_stats = {
        "indexed_document_count": 0,
        "indexed_document_volume": 0,
        "deleted_document_count": 0,
...
@pytest.mark.asyncio
async def test_sync_job_runner_sets_features_for_data_provider() -> None:
    sync_job_runner = create_runner()
