...
def successful_action_log_message(doc_id, action, result) -> str:
...
@patch("connectors.es.sink.CANCELATION_TIMEOUT", -1)
@pytest.mark.asyncio
async def test_prepare_content_index(mock_responses) -> None:
    language_code = "en"
    config = {"host": "http://nowhere.com:9200", "user": "tarek", "password": "blah"}
    headers = {"X-Elastic-Product": "Elasticsearch"}
    index_name = "search-new-index"
...
def index_operation(doc) -> Dict[str, Any]:
...
def update_operation(doc) -> Dict[str, Any]:
    # deepcopy as get_docs mutates docs
    doc_copy = deepcopy(doc)
    doc_id = str(doc_copy.pop("_id"))
    doc_copy["id"] = doc_id
...
def delete_operation(doc) -> Dict[str, Any]:
...
async def basic_rule_engine_mock(return_values) -> Mock:
...
def test_bulk_populate_stats(res, expected_result) -> None:
    sink = Sink(
        client=None,
        queue=None,
        error_monitor=Mock(),
...
@pytest.mark.asyncio
async def test_batch_bulk_with_retry() -> None:
    config = {
        "username": "elastic",
        "password": "changeme",
        "host": "http://nowhere.com:9200",
...
@pytest.mark.asyncio
async def test_batch_bulk_with_errors(patch_logger) -> None:
    config = {
        "username": "elastic",
        "password": "changeme",
        "host": "http://nowhere.com:9200",
...
@pytest.mark.asyncio
async def test_force_canceled_extractor_with_other_errors(patch_logger) -> None:
    queue = Mock()
    queue.put = AsyncMock()
    extractor = Extractor(
        None,
...
@pytest.mark.asyncio
async def test_force_canceled_sink_with_other_errors(patch_logger) -> None:
    queue = Mock()
...
@pytest.mark.asyncio
async def test_cancel_sync(extractor_task_done, sink_task_done, force_cancel) -> None:
    config = {"host": "http://nowhere.com:9200", "user": "tarek", "password": "blah"}
    es = SyncOrchestrator(config)
    es._extractor = Mock()
...
@pytest.mark.asyncio
async def test_should_log_error_when_id_is_missing(patch_logger) -> None:
    operations = []
    client = Mock()
    # item missing id
...
@pytest.mark.asyncio
async def test_should_log_error_when_unknown_action_item_returned(patch_logger) -> None:
    operations = []
    client = Mock()
