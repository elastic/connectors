...
#
"""Shared Microsoft Graph / Entra building blocks.

This module hosts the reusable authentication and HTTP plumbing used to talk to
Microsoft Graph. It is shared between connectors that integrate with Microsoft
services (for example SharePoint Online and Microsoft Teams) so that token
management, pagination, throttling and retry behavior are implemented once.
"""
...
if "OVERRIDE_URL" in os.environ:
    logger.warning("x" * 50)
    logger.warning(
        f"MICROSOFT GRAPH CALLS ARE REDIRECTED TO {os.environ['OVERRIDE_URL']}"
    )
    logger.warning("IT'S SUPPOSED TO BE USED ONLY FOR TESTING")
    logger.warning("x" * 50)
    GRAPH_API_AUTH_URL = os.environ["OVERRIDE_URL"]
else:
...
# Microsoft Graph API pagination constants
...
class BadRequestError(Exception):
    ...
    be translated as empty results, and let us return []."""
...
class MicrosoftSecurityToken:
    ...
    async def get(self):
        ...
        try:
            ...
            match e.status:
                case 400:
                    msg = "Failed to authorize to Microsoft services. Please verify, that provided Tenant Id, Tenant Name and Client ID are valid."
                    raise TokenFetchFailed(msg) from e
                case 401:
                    msg = "Failed to authorize to Microsoft services. Please verify, that provided Secret Value is valid."
                    raise TokenFetchFailed(msg) from e
                case _:
                    msg = f"Failed to authorize to Microsoft services. Response Status: {e.status}, Message: {e.message}"
                    raise TokenFetchFailed(msg) from e
...
class EntraAPIToken(MicrosoftSecurityToken):
    """Token to connect to Microsoft Graph API endpoints using certificate credentials."""
...
class MicrosoftAPISession:
    ...
    async def _post(self, absolute_url, payload=None, retry_count=0):
        try:
            ...
            self._logger.debug(f"Posting to Microsoft Endpoint: {absolute_url}.")
    ...
    async def _get(self, absolute_url, retry_count=0):
        try:
            token = await self._api_token.get()
            headers = {"authorization": f"Bearer {token}"}
            self._logger.debug(f"Calling Microsoft Endpoint: {absolute_url}")
    ...
    async def _handle_client_response_error(self, absolute_url, e, retry_count):
        if e.status == 429 or e.status == 503:
            ...
            if "Retry-After" in response_headers:
                ...
                self._logger.warning(
                    f"Response Code from Microsoft Graph is {e.status} but Retry-After header is not found, using default retry time: {DEFAULT_RETRY_SECONDS} seconds"
                )
            ...
            self._logger.warning(
                f"Rate Limited by Microsoft: a new attempt will be performed in {retry_seconds} seconds (retry-after header: {retry_after}, retry_count: {retry_count}, backoff: {DEFAULT_BACKOFF_MULTIPLIER})"
            )
            ...
            msg = f"Received Unauthorized response for {absolute_url}.\nVerify that the correct Graph API and Microsoft permissions are granted to the app and admin consent is given. If the permissions and consent are correct, wait for several minutes and try again."
    ...
    def _compute_retry_after(self, retry_after, retry_count, backoff):
        # Wait for what the API asks after the first failure.
