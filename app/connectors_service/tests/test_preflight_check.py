...
@pytest.mark.asyncio
@patch("connectors.preflight_check.logger")
async def test_non_native_config_is_not_warned_when_no_service_service_types_are_provided(
...
@pytest.mark.asyncio
@patch("connectors.preflight_check.logger")
async def test_non_native_config_is_warned_when_service_service_types_are_provided(
    patched_logger, mock_responses
):
    mock_es_info(mock_responses)
    mock_index_exists(mock_responses, CONCRETE_CONNECTORS_INDEX)
