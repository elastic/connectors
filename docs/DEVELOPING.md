...
3. [Configure Oracle connector](#configure-oracle-connector)
...
Name of the Oracle database. Default value is `xe`. Examples:
...
- If the value of system change number (SCN) of table in between `min(SCN)` and `max(SCN)` of table `SMON_SCN_TIME`, then the connector will not be able to fetch last updated time and data will index in every sync. For more details check out the following [documentation][reference](https://community.oracle.com/tech/apps-infra/discussion/4076446/show-error-about-ora-08181-specified-number-is-not-a-valid-system-change-number-when-using-scn-t)
...
3. [Configure Microsoft SQL connector](#configure-microsoft-sql-connector)
