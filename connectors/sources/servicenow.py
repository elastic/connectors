...
def load_mapping(role_mapping):
    def _load_json(json_string):
        try:
            ...
            return json_object
        except fastjsonschema.JsonSchemaValueException as exception:
            msg = f"{json_string} is not a valid format. Exception: {exception}"
            raise ConfigurableFieldValueError(msg) from exception
        except ValueError as exception:
            msg = f"{json_string} is not a valid JSON. Exception: {exception}"
            raise ConfigurableFieldValueError(msg) from exception

    json_role_mapping = _load_json(role_mapping)

    if isinstance(json_role_mapping, dict):
        return json_role_mapping
    elif isinstance(json_role_mapping, str):
        # Handle case of escaped json string from the user input,
        # in that case we need to call json.loads() twice
        return _load_json(json_role_mapping)
    else:
        msg = f"{role_mapping} is not a valid JSON."
...
class ServiceNowClient:
    ...
    def __init__(self, configuration):
        ...
        self.role_mapping = self.configuration["service_roles_mapping"]
        self._logger = logger
...
class ServiceNowDataSource(BaseDataSource):
    ...
    def get_default_configuration(cls):
        return {
            ...
            "services": {
                ...
                "order": 4,
                "tooltip": "List of services is ignored when Advanced Sync Rules or Document Level Security are used.",
                "type": "list",
            },
            "retry_count": {
            ...
            "service_roles_mapping": {
                "depends_on": [{"field": "use_document_level_security", "value": True}],
                "display": "textarea",
                "label": "Mapping of Roles and Services",
                "order": 9,
                "type": "str",
            },
        }
    ...
    async def _remote_validation(self):
        ...
        """
        if self.servicenow_client.configuration["use_document_level_security"]:
            if self.servicenow_client.role_mapping != "*":
                self.servicenow_client.role_mapping = load_mapping(
                    self.servicenow_client.role_mapping
                )
            else:
                self.servicenow_client.role_mapping = DEFAULT_SERVICE_ROLES
            self.servicenow_client.services = list(
                self.servicenow_client.role_mapping.keys()
            )
    ...
    async def _fetch_access_controls(self, table_name):
        ...
        for role in self.servicenow_client.role_mapping.get(table_name, []):
            async for user in self._fetch_users_by_roles(roles.get(role)):
    ...
    async def get_docs(self, filtering=None):
        ...
        if filtering and filtering.has_advanced_rules():
            ...
            for label, service_name in (
                self.servicenow_mapping.items() or DEFAULT_SERVICE_MAPPING.items()
            ):
                table_access_control = []
                if self._dls_enabled():
