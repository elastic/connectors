...
async def verify(name, size):
    index_name = f"search-{name}"
    es_index = ESIndexAssertionHelper(index_name)

    try:
        print(f"Verifying {index_name}...")

        await es_index.assert_has_doc_count(size)
        await es_index.assert_pipeline_ran()
        await es_index.assert_content_extraction_successful()
        await es_index.assert_first_doc_is_correct()
