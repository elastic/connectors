...
from aiohttp.client_exceptions import ServerTimeoutError, ClientResponseError, ServerDisconnectedError
...
def _prefix_user_id(user_id: Union[str, int]) -> str:
...
class DropboxClient:
    ...
    async def _set_access_token(self) -> Iterator[None]:
    ...
    def check_errors(self, response: Dict[str, str]):
    ...
    def _get_request_headers(self, file_type: Optional[str], url_name: str, **kwargs) -> Dict[str, str]:
    ...
    def _get_retry_after(self, retry: int, exception: Union[ClientResponseError, Exception, ServerDisconnectedError, ServerTimeoutError]) -> Tuple[int, int]:
    ...
    async def _handle_client_errors(self, retry: int, exception: ClientResponseError) -> Generator[Task, None, int]:
    ...
    async def api_call(self, base_url: str, url_name: str, data: Optional[str]=None, file_type: None=None, **kwargs) -> Iterator[Optional[Task]]:
        retry = 1
    ...
    async def _paginated_api_call(
        self, base_url: str, breaking_field: str, continue_endpoint: Optional[str]=None, **kwargs
    ) -> None:
    ...
    async def ping(self, endpoint: str) -> Iterator[Optional[Task]]:
        return await anext(
            self.api_call(
...
class DropBoxAdvancedRulesValidator(AdvancedRulesValidator):
    ...
    def __init__(self, source: "DropboxDataSource") -> None:
        self.source = source

    async def validate(self, advanced_rules: List[Dict[str, Union[str, Dict[str, Union[str, Dict[str, str]]]]]]) -> SyncRuleValidationResult:
    ...
    async def _remote_validation(self, advanced_rules: List[Dict[str, Union[str, Dict[str, Union[str, Dict[str, str]]]]]]) -> SyncRuleValidationResult:
        try:
...
class DropboxDataSource(BaseDataSource):
    ...
    def _decorate_with_access_control(self, document: Dict[str, Union[str, int, List[str]]], access_control: List[str]) -> Dict[str, Union[str, int, List[str]]]:
    ...
    async def _user_access_control_doc(self, user: Dict[str, Dict[str, Union[str, Dict[str, str], List[str]]]]) -> Dict[str, Union[str, Dict[str, str], Dict[str, Dict[str, Union[Dict[str, List[str]], str]]]]]:
    ...
    async def validate_config(self) -> Iterator[Optional[Task]]:
    ...
    async def _remote_validation(self) -> Iterator[Optional[Task]]:
        try:
    ...
    async def set_user_info(self) -> Iterator[Optional[Task]]:
        try:
            endpoint = EndpointName.AUTHENTICATED_ADMIN.value
    ...
    async def get_content(
        self, attachment: Dict[str, Union[str, int, bool]], is_shared: bool=False, folder_id: None=None, timestamp: None=None, doit: bool=False
    ...
    def download_func(self, is_shared: bool, attachment: Dict[str, Union[str, int, bool]], filename: str, folder_id: None) -> Optional[partial]:
        if is_shared:
            return partial(
    ...
    def _adapt_dropbox_doc_to_es_doc(self, response: Dict[str, Union[str, int, bool]]) -> Dict[str, Union[str, int]]:
    ...
    def _adapt_dropbox_shared_file_doc_to_es_doc(self, response: Dict[str, Union[str, int, bool]]) -> Dict[str, Union[str, int]]:
        return {
            "_id": response.get("id"),
            "type": FILE,
            "name": response.get("name"),
    ...
    async def advanced_sync(self, rule: Dict[str, Union[Dict[str, List[str]], str, Dict[str, List[Dict[str, str]]]]]) -> None:
    ...
    def get_group_id(self, permission: Dict[str, Union[Dict[str, Union[str, int]], Dict[str, str]]], identity: str) -> str:
        if identity in permission:
            return permission.get(identity).get("group_id")

    def get_email(self, permission: Dict[str, Dict[str, str]], identity: str) -> str:
        if identity in permission:
            return permission.get(identity).get("email")

    async def get_permission(self, permission: Dict[str, Union[List[Union[Any, Dict[str, Dict[str, Union[str, int]]]]], List[Dict[str, Dict[str, str]]], str, bool]], account_id: int) -> List[str]:
        permissions = []
    ...
    async def get_folder_permission(self, shared_folder_id: int, account_id: int) -> List[str]:
        if (
    ...
    async def get_file_permission_without_batching(self, file_id: int, account_id: int) -> List[str]:
        async for (
            permission
    ...
    async def get_account_details(self) -> Tuple[str, str]:
    ...
    async def get_permission_list(self, item_type: str, item: Dict[str, int], account_id: int) -> List[str]:
        if item_type == FOLDER:
    ...
    def document_tuple(self, document: Dict[str, Union[str, int, List[str]]], attachment: Union[Dict[str, Union[str, int, bool]], str, Dict[str, Union[str, int]]], folder_id: None=None) -> Union[Tuple[Dict[str, Union[int, str]], partial], Tuple[Dict[str, Union[int, str]], None], Tuple[Dict[str, Union[int, str, List[str]]], partial]]:
