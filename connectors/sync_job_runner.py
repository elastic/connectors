...
class InsufficientESLicenseError(Exception):
    def __init__(self, required_license: License, actual_license: License) -> None:
        super().__init__(
...
class SyncJobRunner:
    ...
    def __init__(
        ...
        sync_job: Mock,
        connector: Mock,
        es_config: Dict[Any, Any],
        service_config: Dict[Any, Any],
    ) -> None:
    ...
    async def execute(self) -> Iterator[Union[Task, None, Future]]:
        if self.running:
    ...
    def _data_source_framework_config(self) -> DataSourceFrameworkConfig:
    ...
    async def _sync_done(self, sync_status: connectors.protocol.connectors.JobStatus, sync_error: Optional[Any]=None) -> Iterator[Task]:
    ...
    def log_counters(self, counters: Dict[str, int]) -> None:
        """
    ...
    async def prepare_docs(self) -> None:
    ...
    async def update_ingestion_stats(self, interval: int) -> Iterator[Optional[Future]]:
        while True:
    ...
    def _content_extraction_enabled(self, sync_job_config: Dict[str, Union[str, bool]], pipeline_config: Union[Dict[str, str], connectors.protocol.connectors.Pipeline, Dict[str, bool]]) -> bool:
