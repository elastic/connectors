...
class ESManagementClient(ESClient):
    ...
    async def create_content_index(self, search_index_name: str, language_code: Optional[str]) -> Union[AsyncMock, ApiResponse, ObjectApiResponse]:
    ...
    async def index_exists(self, index_name: str) -> Union[AsyncMock, HeadApiResponse]:
        return await self._retrier.execute_with_retry(
            partial(self.client.indices.exists, index=index_name)
        )

    async def get_index_or_alias(self, index_name: str, ignore_unavailable: bool=False) -> Optional[Union[Dict[str, Dict[str, Dict[Any, Any]]], Dict[str, Dict[Any, Any]]]]:
        """
    ...
    async def bulk_insert(self, operations: List[Union[Dict[str, Dict[str, str]], Dict[str, str], Any]], pipeline: str) -> Generator[Task, None, Union[Dict[str, List[Any]], ObjectApiResponse, Dict[str, Union[bool, List[Dict[str, Dict[str, Union[str, Dict[str, str]]]]]]], Dict[str, List[Dict[str, Dict[str, str]]]]]]:
        return await self._retrier.execute_with_retry(
            partial(
                self.client.bulk,
