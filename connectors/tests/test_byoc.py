...
def mock_server(mock_responses):
    headers = {"X-Elastic-Product": "Elasticsearch"}
    mock_responses.post(
        "http://nowhere.com:9200/.elastic-connectors/_refresh",
        headers=headers,
        repeat=True,
    )
...
@pytest.mark.asyncio
async def test_change_settings(mock_responses, patch_logger):
    _CONNECTORS_CACHE.clear()
    mock_server(mock_responses)
    ...
    try:
        ...
        # simulate a change of the pipeline settings in Kibana
        async for connector in connectors.get_list():
            await connector.sync(Data(), es, 0)
            await connector.close()
