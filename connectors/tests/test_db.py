...
async def test_connector_prepare_with_prepared_connector():
    ...
    config = {
        "connector_id": doc_id,
        "service_type": "banana",
        "sources": {"banana": "connectors.tests.test_db:Banana"},
    }
    index = Mock()
    index.fetch_response_by_id = AsyncMock(return_value=connector_doc)
    index.update = AsyncMock()
...
@patch("connectors.db.SyncJobIndex.index")
...
@pytest.mark.asyncio
@patch("connectors.db.SyncJobIndex.get_all_docs")
